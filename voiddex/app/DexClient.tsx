...
export type DexPoolState={fee:string|null;reserve0:string;reserve1:string;totalSupply:string;balance:string};
...
export default function VoidDex({initialStates}:{initialStates:DexPoolState[]}){
 ...
 async function sponsor(label:string,target:Address,data:Hex,spends:Array<{token:Address;amount:bigint}>){
  ...
  try {
   ...
   for (const limit of limits.values()) {
    ...
    permits.push({token:limit.token,spender:limit.spender,value:limit.value,deadline,...split(permitSignature)});
   }
 ...
 return <main className="shell"><header className="top"><a className="brand" href="/">VOID<b>DEX</b></a><button className="wallet" onClick={connect}>{account?`${account.slice(0,6)}…${account.slice(-4)}`:'Connect wallet'}</button></header><section className="hero"><div><div className="tag">UNISWAP V2 · VOID CHAIN #1 · TESTNET</div><h1>Swap on<br/>your <b>chain.</b></h1><p>Every action below is a signed VOID-sponsored request. Your wallet signs bounded permits; it never sends an ETH transaction. The relayer pays parent-chain gas and the Paymaster charges the chain fee in VOID: 98% to the Deed holder and 2% to the protocol.</p></div><aside className="fee"><span className="label">Chain fee per action</span><strong>{feeReady ? `${show(fee)} VOID` : 'Unavailable'}</strong><small>Gas budget: up to {show(MAX_GAS_VOID)} VOID in addition to the chain fee. Unused gas budget is refunded. The cap is not the final charge.</small></aside></section>{!feeReady && <p className="status">Trading paused: the fee oracle is unavailable. Pool balances remain visible.</p>}<section className="grid"><div className="main"><nav className="tabs">{PAIRS.map((p,i)=><button className={i===pairI?'on':''} onClick={()=>selectPair(i)} key={p.address}>{p.label}</button>)}</nav><section className="card"><div className="head"><h2>Swap</h2><span>0.50% max slippage</span></div><label className="input"><span className="label">You pay · {dir?s0:s1}</span><input value={amount} onChange={e=>setAmount(e.target.value)} inputMode="decimal"/></label><button className="mutedBtn" onClick={()=>setDir(!dir)}>Reverse direction</button><div className="input"><span className="label">You receive · {dir?s1:s0}</span><strong>{show(quote)}</strong><small>Minimum {show(quote*9950n/10000n)}</small></div><button className="action" disabled={!!busy || !feeReady} onClick={swap}>{busy==='swap'?'Confirming…':`Sign swap · ${show(fee)} VOID`}</button></section><section className="card"><div className="head"><h2>Pool</h2><span>LPs retain 0.30% swap fee</span></div><div className="two"><input value={a0} onChange={e=>setA0(e.target.value)} inputMode="decimal" aria-label={s0}/><input value={a1} onChange={e=>setA1(e.target.value)} inputMode="decimal" aria-label={s1}/></div><button className="action" disabled={!!busy || !feeReady} onClick={add}>{busy==='liquidity'?'Confirming…':`Sign liquidity · ${show(fee)} VOID`}</button><div className="remove"><input value={burn} onChange={e=>setBurn(e.target.value)} placeholder="LP shares to remove" inputMode="decimal"/><button className="mutedBtn" disabled={!!busy || !feeReady} onClick={remove}>{busy==='remove'?'Confirming…':'Sign remove'}</button></div></section></div><aside className="side"><section className="card"><div className="head"><h2>Pool state</h2><span>live</span></div><div className="metric"><span>{s0} reserve</span><b>{show(r0)}</b></div><div className="metric"><span>{s1} reserve</span><b>{show(r1)}</b></div><div className="metric"><span>Total LP</span><b>{show(lp)}</b></div><div className="metric"><span>Your LP</span><b>{show(mine)}</b></div></section><section className="card notice"><b>Test assets</b><p>tUSD and tLINK are local test tokens. Claiming them is a paid Chain #1 action, so it appears in VoidScan and pays the VOID fee.</p><button className="mutedBtn" disabled={!!busy || !feeReady} onClick={claim}>{busy==='claim'?'Confirming…':`Sign claim · ${show(fee)} VOID`}</button></section></aside></section>{note&&<p className="status">{note}</p>}<footer className="foot"><span>Factory {DEX.factory.slice(0, 6)}…{DEX.factory.slice(-4)}</span><span>Runtime {RUNTIME}</span><a href="https://voidscan-nu.vercel.app">Open VoidScan →</a></footer></main>;
}
