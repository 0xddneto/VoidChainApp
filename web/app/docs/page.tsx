...
import { CONTRACTS, EXPLORER, MANIFEST_HASH, RELEASE, SOURCE_REPOSITORY } from '@/lib/public-release';
  ...
  return <>
    ...
    <main className={styles.wrap}>
      ...
      <article className={styles.docs}>
        ...
        <section id="governance"><h2>One DAO per Deed</h2><p>The first owner sets the initial chain transaction fee during activation. From that point onward, only the chain DAO may change the fee or application publishing policy. This cannot be returned to unilateral owner control.</p><p>The current owner may propose any subject, including a signalling proposal with no executable action. Voting lasts five days. Voting power is the wallet's VOID balance at the previous-block snapshot; VOID is never staked or locked. A proposal needs 1% of eligible circulating VOID and more votes for than against.</p><p>An executable proposal does not receive protocol-wide authority: each target enforces its own caller permissions. The Runtime accepts a DAO only for the same Deed; Paymaster, Treasury, timelock and sibling chains reject it. Wallet-balance voting deliberately has no custody lock, so multi-block borrowed or concentrated voting power remains a disclosed governance risk.</p></section>
        ...
        <section id="contracts"><h2>Current testnet contracts</h2><p>{RELEASE} · Manifest <code>{MANIFEST_HASH}</code></p><dl className={styles.contracts}>{Object.entries(CONTRACTS).map(([name, address]) => <div key={name}><dt>{name}</dt><dd><a href={`${EXPLORER}/address/${getAddress(address)}`} target="_blank" rel="noopener noreferrer">{getAddress(address)}</a></dd></div>)}</dl><p>Network: Robinhood Chain Testnet · EIP-155 chain ID 46630. <a href="/contracts">Contract directory and exact source</a> · <a href="/security">Security and live authority checks</a> · <a href={SOURCE_REPOSITORY} target="_blank" rel="noopener noreferrer">Public GitHub repository</a>. This page, transaction UI and machine-readable API all import the same release manifest.</p></section>

        <section id="validation"><h2>Load validation</h2><p>The V11 source release passed 338 local EVM tests across 40 suites. The load coverage includes 1,000 provisioned users and 4,800 mixed application calls, plus a dedicated zero-ETH run with 256 distinct users completing 1,024 sponsored calls without Runtime or Paymaster allowances.</p><p>Stateful accounting invariants completed 256,000 handler calls with no reverts or fee-conservation, custody, revenue-isolation or protocol-split violation. The 1,111-chain activation and independent-revenue scale checks also passed.</p><div className={styles.callout}><b>Public testnet canary</b><span>The V11 deployment was accepted only after bytecode verification, exact migration accounting, six registered apps, two live DEX pools and all 1,111 DAOs were checked on-chain. High-volume figures are deterministic local-EVM evidence; they are not a public-RPC throughput benchmark or an external audit.</span></div></section>

        <section id="status"><h2>Safety status</h2><p>This is pre-audit testnet software. V11 freezes the runtime oracle and VOID protocol operators after initial configuration and places Paymaster and Treasury administration behind a public 48-hour timelock. The relays use persistent nonce admission and the explorer delays publication for parent-chain confirmations. Local tests, verified bytecode and live acceptance transactions are engineering evidence, not a third-party security audit or a mainnet-readiness statement. Testnet VOID has no monetary value. Production still requires an external audit, a hardware-backed multisig proposer, independent monitoring, tested backups and a fresh deployment review.</p></section>
      </article>
    </main>
  </>;
}
