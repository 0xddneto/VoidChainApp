...
VOID needs no allowance on V11: only the permanently frozen Paymaster and
Runtime can use the token's protocol transfer path, and only the signed request
opens an exact budget. External EIP-2612 assets may still need a permit to the
Runtime. The exact per-action budget is never inferred
...
exists. In V11 these calls must first be scheduled in the public 48-hour
...
The V11 Robinhood **testnet** deployment uses its permanently locked genesis
...
mainnet solvency. The V11 testnet timelock proposer is still one project wallet;
